...
for(c in top.coords){
    ...
    initial.name <- name.balance(phy_tree(pseq), tax_table(pseq), c)
    
    if(grepl("Kingdom", initial.name)){
        print("------------")
        print(c)
        votes <- name.balance(phy_tree(pseq), tax_table(pseq), c, return.votes=c("up", "down"))
        print("up:")
        print(votes$up.votes)
        print("down:")
        print(rbindvotes$down.votes)
    }
}
